// components/icons — the app's own SVG icon family and the single registry every
// surface imports from. Hand-drawn on a 24px grid, stroked with `currentColor`
// (rounded caps/joins) so every icon inherits colour + size from its context and
// feels part of one system rather than a stock set. A few domain glyphs carry a
// subtle accent-tinted fill for a more tactile, premium read. Zero third-party
// icon dependencies, app-wide. Named exports → tree-shakeable (only the glyphs a
// chunk actually imports ship in that chunk).
//
// `components/ui/icons.tsx` is a thin back-compat re-export of this module, so the
// ~77 existing `../ui/icons` importers keep working unchanged; new code should
...
export const IconCamera = (p: IconProps) => (<Glyph {...p}><path d="M3 9a2 2 0 0 1 2-2h.93a2 2 0 0 0 1.66-.9l.82-1.2A2 2 0 0 1 10.07 4h3.86a2 2 0 0 1 1.66.9l.82 1.2A2 2 0 0 0 18.07 7H19a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" /><circle cx="12" cy="13" r="3" /></Glyph>)
export const IconMic = (p: IconProps) => (<Glyph {...p}><rect x="9" y="2" width="6" height="11" rx="3" /><path d="M5 10v2a7 7 0 0 0 14 0v-2" /><line x1="12" y1="19" x2="12" y2="22" /><line x1="8" y1="22" x2="16" y2="22" /></Glyph>)
...
/** Sun — light theme: a filled core with eight rays, on the 24px grid. */
export const IconSun = (p: IconProps) => (
  <Glyph {...p}><circle cx="12" cy="12" r="4" /><path d="M12 2.2v2.4M12 19.4v2.4M4.4 4.4l1.7 1.7M17.9 17.9l1.7 1.7M2.2 12h2.4M19.4 12h2.4M4.4 19.6l1.7-1.7M17.9 6.1l1.7-1.7" /></Glyph>
)

/** Moon — dark theme: a crescent (a disc masked by an offset arc). */
export const IconMoon = (p: IconProps) => (
  <Glyph {...p}><path d="M20.5 14.3A8.2 8.2 0 0 1 9.7 3.5a8.2 8.2 0 1 0 10.8 10.8Z" /></Glyph>
)

// ─── Sharing glyph ────────────────────────────────────────────────────────────

/** Share — the three-node share graph (one source linked out to two recipients). */
export const IconShare = (p: IconProps) => (
  <Glyph {...p}><circle cx="6" cy="12" r="2.7" /><circle cx="17" cy="5.6" r="2.7" /><circle cx="17" cy="18.4" r="2.7" /><path d="m8.35 10.7 6.3-3.7M8.35 13.3l6.3 3.7" /></Glyph>
)

// ─── Multi-agent import glyphs (pre-cut for F3: the agentic-import surface) ────
// One glyph per concept in the import brain's live view — an autonomous agent,
// a pipeline stage, an escalation to a stronger model, adversarial verification,
// reconciliation of parallel results, cross-model disagreement, and the SSE
// progress stream. Same 24-grid / currentColor system as the rest of the family.

/** Agent — an autonomous worker: a bot head with an antenna and two eyes. */
export const IconAgent = (p: IconProps) => (
  <Glyph {...p}><rect x="4.8" y="7.6" width="14.4" height="11" rx="3.2" /><path d="M12 4.4v3.2" /><circle cx="12" cy="3.8" r="1.15" fill="currentColor" stroke="none" /><circle cx="9.4" cy="12.8" r="1.15" fill="currentColor" stroke="none" /><circle cx="14.6" cy="12.8" r="1.15" fill="currentColor" stroke="none" /><path d="M3.4 12.4v2.2M20.6 12.4v2.2" /></Glyph>
)

/** Stage — a pipeline milestone: a flag on a staff. */
export const IconStage = (p: IconProps) => (
  <Glyph {...p}><path d="M6 3.2v17.6" /><path d="M6 4.6h11.2l-2.5 3.3 2.5 3.3H6" /></Glyph>
)

/** Escalate — hand off to a stronger tier: a double chevron rising. */
export const IconEscalate = (p: IconProps) => (
  <Glyph {...p}><path d="m6.5 12.5 5.5-5.5 5.5 5.5" /><path d="m6.5 18 5.5-5.5 5.5 5.5" /></Glyph>
)

/** Verify — an adversarial check: a checked medal with a ribbon. */
export const IconVerify = (p: IconProps) => (
  <Glyph {...p}><circle cx="12" cy="10" r="6" /><path d="m9.2 9.9 2 2 3.7-4.1" /><path d="M8.6 15.2 7.2 20.8 12 18.5l4.8 2.3-1.4-5.6" /></Glyph>
)

/** Reconcile — merge parallel results: two branches joining one output arrow. */
export const IconReconcile = (p: IconProps) => (
  <Glyph {...p}><path d="M5.5 3.5v4.2a4 4 0 0 0 4 4h8.5" /><path d="M18.5 3.5v4.2a4 4 0 0 1-4 4" /><path d="m15.5 8.7 3 3-3 3" /></Glyph>
)

/** Disagreement — cross-model divergence: two arrows pushing apart at a seam. */
export const IconDisagreement = (p: IconProps) => (
  <Glyph {...p}><path d="M12 4.5v3M12 10.8v2.4M12 16.5v3" /><path d="m7.6 8.2-3 3.8 3 3.8" /><path d="m16.4 8.2 3 3.8-3 3.8" /></Glyph>
)

/** Stream — a live SSE progress feed: three flowing signal lines. */
export const IconStream = (p: IconProps) => (
  <Glyph {...p}><path d="M3.6 8c2.4-2 4.3 2 6.6 0s4.3-2 6.6 0 1.6.9 1.6.9" /><path d="M3.6 12c2.4-2 4.3 2 6.6 0s4.3-2 6.6 0" /><path d="M3.6 16c2.4-2 4.3 2 6.6 0s4.3-2 6.6 0" /></Glyph>
)
