...
// criticality (product health) → stable remainder. Read-only everywhere: rows only
// navigate to the Tasks board, so a VIEWER sees the same inquiry surface as anyone.
import { useMemo, useState } from 'react'
import { useNavigate } from 'react-router-dom'
import { IconTasks, IconArrowRight, IconWarning, IconCheckCircle } from '../ui/icons'
import { Badge } from '../ui'
import { healthColor } from '../../lib/productHealth'
import {
  prioritize, withinHorizon, toMillis, daysUntil,
  type Horizon, type Criticality,
} from '../../lib/homePriorities'
...
export function PriorityRail({ status, tasks, products, now }: Props) {
  ...
  // Product lookups: name for the chip, health as the criticality signal.
  const productById = useMemo(() => new Map(products.map(p => [p.id, p])), [products])
  const healthOf = useMemo(
    () => (t: { productId?: string }): Criticality | null => {
      const p = t.productId ? productById.get(t.productId) : undefined
      return p?.health ? { score: p.health.score, findingCount: p.health.findingCount } : null
    },
    [productById],
  )
  ...
  const ranked = useMemo(() => {
    ...
    return prioritize(inWindow, healthOf)   // absolute due-date order — no `now` needed
  }, [tasks, horizon, now, healthOf])
      ...
      {status === 'loading' ? (
        ...
        <ul className="flex flex-col gap-1.5">
          {ranked.map(t => {
            ...
            const score = product?.health?.score
            return (
              <li key={t.id}>
                <button onClick={() => navigate('/app/tasks')}
                  ...
                  <div className="flex items-center gap-2">
                    {/* Criticality dot — the product's health; only shown when at risk (< 100). */}
                    {typeof score === 'number' && score < 100 && (
                      <span className="shrink-0 w-1.5 h-1.5 rounded-full" style={{ background: healthColor(score) }}
                        title={`Product health ${score}`} aria-label={`Product health ${score}`} />
                    )}
