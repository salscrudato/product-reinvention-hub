...
// of the product. Footer carries a health chip and the AI-summary affordance. No nested
// interactive elements: the container is a div; each region is its own button. Tiles are
...
export function ProductCard({ p }: { p: WithId<Product> }) {
  ...
  return (
    <div
      className="group relative h-full bg-surface rounded-[16px] overflow-hidden flex flex-col shadow-[var(--shadow-card)] hover:shadow-[var(--shadow-card-hover)] hover:-translate-y-0.5 transition-all duration-200"
      ...
      <div className="p-5 flex flex-col gap-4 flex-1">
        ...
        <button onClick={() => go()} aria-label={`Open ${p.name}`}
          ...
          <p className="text-xs text-dim tnum">
            {stateLabel}{p.marketSegment ? <> · {p.marketSegment}</> : null}
          </p>
        </button>
        ...
        <div className="grid grid-cols-5 gap-1.5">
          {TILES.map(({ key, label, Icon }) => (
            <button key={key} onClick={() => go(key)} aria-label={`${p.name} — ${label}`}
        ...
        {/* Footer — health + AI summary */}
        <div className="flex items-center gap-3 text-xs pt-3 mt-auto" style={{ borderTop: '1px solid var(--color-border)' }}>
          <span className="inline-flex items-center gap-1.5">
            <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: healthColor }} aria-hidden="true" />
            <span className="text-dim tnum">Health {health}{findings ? ` · ${findings} finding${findings === 1 ? '' : 's'}` : ''}</span>
          </span>
          <button onClick={() => go('overview')} aria-label={`${p.name} — AI summary`}
            className="ml-auto shrink-0 inline-flex items-center gap-1 pl-1.5 pr-2 py-1 rounded-[7px] text-[11px] font-medium text-accent hover:bg-accent-soft transition-colors focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-accent">
