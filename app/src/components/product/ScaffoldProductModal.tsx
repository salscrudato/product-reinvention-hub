...
import { resolveLobByRefId, DEFAULT_LOB, DEFAULT_TASK_TEMPLATES } from '@pf/shared'
import type {
...
export function ScaffoldProductModal({ onClose, onCreated }: Props) {
  ...
  const toggleTask = (i: number) => setTaskChecked(prev => {
    const n = new Set(prev); if (n.has(i)) n.delete(i); else n.add(i); return n
  })
  const setAllTasks = (on: boolean) => setTaskChecked(on ? new Set(DEFAULT_TASK_TEMPLATES.map((_, i) => i)) : new Set())
  ...
  async function run() {
    ...
    try {
      ...
      setChecked(next)
      setTaskChecked(new Set(DEFAULT_TASK_TEMPLATES.map((_, i) => i)))
      setPhase('review')
    } catch (e) {
  ...
  async function create() {
    ...
    const chosenTaskCount = DEFAULT_TASK_TEMPLATES.filter((_, i) => taskChecked.has(i)).length
    const total = 1 + chosenForms.length + chosenCovs.length + chosenRules.length + chosenTaskCount
    ...
    try {
      ...
      // 5) Lifecycle tasks — only the selected ones, SLA-driven due dates off today.
      const startDate = new Date()
      let torder = 0
      for (let i = 0; i < DEFAULT_TASK_TEMPLATES.length; i++) {
        if (!taskChecked.has(i)) continue
        const tmpl  = DEFAULT_TASK_TEMPLATES[i]!
        const dueAt = new Date(startDate)
        dueAt.setDate(dueAt.getDate() + tmpl.daysOffset)
        await adapter.db.mutate({
          op: 'create', path: `tasks/task-${draftId}-${i}`,
          entityType: 'task', productId: draftId, actor,
          data: {
            title: tmpl.title, column: tmpl.column, productId: draftId,
            checklist: [], order: torder++, dueAt: dueAt.toISOString(),
            status: 'ACTIVE', lifecycle: 'DRAFT', reviewStatus: 'NOT_STARTED',
          },
        })
        done++; tick(tmpl.title)
      }
    ...
    <Dialog open title="Scaffold a product with AI" onClose={onClose} width="max-w-2xl">
      {!canEdit ? (
        ...
        <ScaffoldReview
          ...
          taskChecked={taskChecked} onToggleTask={toggleTask} onSetAllTasks={setAllTasks}
...
function ScaffoldReview({ plan, name, setName, isOn, toggle, taskChecked, onToggleTask, onSetAllTasks, onCancel, onCreate }: {
  plan: ScaffoldPlan
  name: string; setName: (s: string) => void
  isOn: (k: string, i: number) => boolean
  toggle: (k: string, i: number) => void
  taskChecked: Set<number>
  onToggleTask: (i: number) => void
  onSetAllTasks: (on: boolean) => void
  ...
  const selected = count('cov', plan.coverages.items.length) + count('form', plan.forms.items.length) + count('rule', plan.rules.items.length) + taskChecked.size
  const totalItems = plan.coverages.items.length + plan.forms.items.length + plan.rules.items.length + DEFAULT_TASK_TEMPLATES.length
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-5 max-h-[46vh] overflow-y-auto -mx-1 px-1">
        ...
        <Section icon={IconCheckSquare} label="Lifecycle tasks" n={DEFAULT_TASK_TEMPLATES.length} note="Standard product-manufacture tasks. Deselect any that don't apply.">
          <TaskSelectList templates={DEFAULT_TASK_TEMPLATES} checked={taskChecked} onToggle={onToggleTask} onSetAll={onSetAllTasks} />
        </Section>
      </div>

      <div className="flex items-center justify-between gap-2 pt-1">
        <span className="text-xs text-faint">{selected} of {totalItems} selected</span>
