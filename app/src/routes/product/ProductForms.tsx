...
function FormDrawer({ form, onClose }: { form: WithId<Form>; onClose: () => void }) {
  return (
    <Drawer open title={`${form.number} — ${form.name}`} onClose={onClose} width="w-[480px]">
      <div className="flex flex-col gap-5">
        {/* Meta */}
        <div className="flex flex-wrap gap-1.5">
          <Badge label={form.category} color={CAT_COLOR[form.category] ?? 'default'} />
...
export default function ProductForms() {
  const { forms, loading } = useProductCtx()
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <span className="text-sm text-faint">{filtered.length} form{filtered.length !== 1 ? 's' : ''}</span>
      </div>
            ...
            <tbody>
              {filtered.map(form => (
                <tr key={form.id} onClick={() => setSelected(form)}
                  ...
                  <td className="px-4 py-3 font-mono text-xs text-text">{form.number}</td>
      ...
      {selected && <FormDrawer form={selected} onClose={() => setSelected(null)} />}
    </div>
  )
}
