...
> Copy everything between the rules below into a fresh coding-agent session that has
> this repository checked out. It is self-contained and repo-specific.
...
people screenshot and say *"why can't our tools look like this?"*. Your taste is
the intersection of **Apple Human Interface Guidelines, Linear, Stripe, Vercel, and
...
it best-in-class. When you are unsure, you choose the more restrained option.
...
application** so that it *wows* users with an elegant, premium, Apple-inspired,
innovative experience — while preserving every domain rule and staying production-safe.
Work **page by page**, but treat the app as one coherent system: shared components,
one type scale, one motion language, one icon family.
...
app/src
  ...
  components/product/ product/coverage feature components
...
3. **Roles via custom claims**, enforced in **Firestore rules AND Functions**, never
   ...
   ADMIN = users/settings/audit. Editing affordances hide for VIEWER but the server
...
- **Innovative, never gimmicky** — the "wow" comes from craft (a self-drawing SVG, a
  live rating flow, an intelligent extraction), not decoration.
...
The tokens live in `app/src/index.css` (`@theme`). Keep the brand and extend the
system; never hard-code hex in components — use the tokens / gradient vars.
...
- **Type** — **Inter** (UI) + **JetBrains Mono** (refIds, form numbers, figures, code).
  ...
  figures (`tnum`); balanced wrapping on headings; optical sizing on. Refine line-height
  and letter-spacing until it reads like a shipping Apple app.
...
1. **Cursor affordance** — every clickable element uses `cursor: pointer` (buttons,
   cards, rows, tiles, chips, tabs, toggles, nav). Add a small base rule so
...
2. **Premium SVGs only** — audit **every** SVG: the icon family
   ...
   map, the rating-flow graphic, and empty-state art. All must be crisp on a 24px grid,
...
5. **Accessibility & keyboard** — AA contrast; visible focus rings (the `focus-ring`
   ...
   tabs, dialogs; `⌘K` palette and `⌘.` feedback keep working; respect reduced motion.
...
7. **Consistency** — reuse `components/ui` primitives; if you need a new pattern
   (segmented control, stat, popover, upload dropzone, stepper), build it **once** in
   `components/ui` and use it everywhere.

---

## 8 · Explicit mandated changes (from the product owner)

These are required, in addition to the general elevation:
...
- Do not lose the signal: fold the essentials into the product header — the meta line
...
**B. Base form gating + AI coverage extraction.** (see full spec in §10)
- A product must have a **base coverage form uploaded** before the **AI Summary /
  extraction** action is enabled. Until then, the action is disabled with a clear,
  friendly hint ("Upload the base form to generate a summary").
- Once a base form exists, the summary/extraction **reads the form and proposes the
  product's coverages** — prefilling as much as possible (names, limits, deductibles,
  attached forms, requirement) — and lets the user **review and deselect** the ones the
  model got wrong before anything is written. Grounded; cite the form.

**C. Coverages view — significant UI/UX enhancement.** The coverage hub cards, the
tile grid, and the Limits/Deductibles/States editors already exist — elevate them
further: refined density and typography, clearer counts and relationships, smoother
transitions opening the editors, better empty/zero states (e.g. "0 Limits → Add your
first"), premium option tables, and a genuinely delightful cards ⇄ list experience.

**D. Pricing page — enhance UI/UX.** Keep the live rating trace (it proves the
`$1,528` derivation — valuable), but make the whole worksheet feel premium: the inputs
panel as a clean, grouped form; the trace as an elegant, legible flow with tasteful
running-total emphasis; refined Flow/Table toggle; crisp refId chips in the trace.
Make the relationship between an input change and the premium feel *alive*.
...
- **App shell** — Sidebar + Topbar + the `⌘K` palette + `⌘.` feedback + the product
  header banner used across tabs. Make the banner elegant and quieter.
- **Home** — the assistant + at-a-glance workspace.
- **Products** — cards ⇄ list (done); refine card, list density, filters, empty states.
...
- **Product › Pricing** — per §8D.
- **Product › States** — the US tile map + footprint editor; premium, correct counts.
- **Product › Rules** — the IF→THEN flow cards + Simulate panel + composer.
- **Builder, Explorer, Tasks, News, Claims, Dictionary, Feedback, Admin, Share** —
  each elevated to the same bar; do not leave any page behind.

---

## 10 · Feature spec — Base Form upload → grounded coverage extraction

Deliver end to end, through the adapter seam and grounded per §4.

1. **Upload** — on a product (Overview header, or a dedicated slot), allow an EDITOR to
   upload the **base coverage form** (PDF or text) to Storage via
   `adapter.storage.upload(...)`. Store a reference on the product (e.g.
   `baseForm: { path, name, uploadedAt, uploadedBy }`) via `mutate()`. Show it as a
   tasteful file chip with replace/remove. VIEWER cannot upload (rules + UI).
2. **Gate** — the **AI Summary / "Extract coverages"** action is **disabled until a
   base form exists**, with a clear tooltip/hint. Once present, it's enabled.
3. **Extraction** — a Cloud Function (extend `functions/src`, e.g. `describe`/`builder`
   or a new `extract`) reads the uploaded form (parse PDF→text server-side) and, using
   Claude via tools, returns a **structured proposal**: coverages with prefilled
   `name`, `requirement`, `premiumGenerating`, candidate limits/deductibles (typed
   `StandardOption`s), and attached form numbers — each with a **confidence** and a
   **citation** back to the form. Never invent; if unsure, mark low confidence.
4. **Review UI** — a review step (dialog/drawer) lists the proposed coverages
   **prefilled and pre-checked**; the user can **deselect wrong ones**, edit values,
   and confirm. Show confidence subtly. Nothing is written until "Add selected".
...
6. **Grounded summary** — the "summary" the button produces cites the base form and
   the extracted refIds; honest about anything it couldn't determine.

Design the whole flow to feel like magic and stay trustworthy: upload → shimmer →
"we found N coverages" → review → confirm.
...
1. **Read** CLAUDE.md + the relevant `docs/` before each area. Screenshot/inspect the
   current surface (run it against emulators).
2. **Critique** against this rubric, scoring each 1–5 and only accepting ≥4.5:
   - **A. Layout & hierarchy** — is the eye led; is there one clear primary action?
...
clickable element has a pointer cursor; Overview has no right rail; the base-form gate
...
- Commit locally per surface with clear messages. **Do NOT deploy to production and do
  ...
  stop and ask first.
- Report at the end with a per-surface before/after summary, the rubric scores, the
  gate result, screenshots, and anything deferred.
