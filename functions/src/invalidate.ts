...
//   • product summary       — the cached summary is reused as-is until the product's ITEM
//     COMPOSITION changes: only ADDING a new item (a coverage / rule / form rule) flags it
//     stale. Plain edits to existing items or product fields keep the cached summary (no
//     churn, no wasted spend); item removals are caught client-side by the composition key.
...
/** Product-scoped kinds — used to bump the dictionary corpus version on any change. */
const PRODUCT_SCOPED = new Set<Kind>(['product', 'coverage', 'rule', 'formRule', 'ratingProgram'])

/** Item kinds whose ADDITION changes the product's composition and so invalidates the
 *  cached summary. Editing an existing item, or the product's own fields, does not —
 *  the summary is cached and reused until a genuinely new item is added. */
const ITEM_KINDS = new Set<Kind>(['coverage', 'rule', 'formRule'])
...
async function markSummaryStale(kind: Kind, params: Record<string, string>, isCreate: boolean): Promise<void> {
  // Only a NEW item (a freshly-created coverage / rule / form rule) invalidates the cached
  // summary. Updates to existing items, the product doc itself, and deletes are all ignored
  // here (deletes are caught client-side by the composition key).
  if (!isCreate || !ITEM_KINDS.has(kind) || !params.pid) return
  try {
