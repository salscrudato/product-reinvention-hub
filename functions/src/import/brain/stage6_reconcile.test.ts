// functions/src/import/brain/stage6_reconcile.test.ts
// Pure-function coverage for reconcileOutput (stage 6). No AI calls, no I/O.

import { describe, it, expect } from 'vitest'
import { reconcileOutput } from './stage6_reconcile'
import type {
  BrainEntity, ClassifiedSheet, HeaderLock, SheetColumnMap,
  ReviewItem, ValidationDiscrepancy,
} from './types'

// ─── Minimal fixture builders ──────────────────────────────────────────────────

function entity(overallConfidence: number, reviewFlag: boolean): BrainEntity {
  return {
    kind: 'coverage',
    fields: [],
    overallConfidence,
    sourceSheet: 'GL Product Framework',
    sourceRowIndex: 1,
    reviewFlag,
...
function sheet(domain: ClassifiedSheet['domain']): ClassifiedSheet {
  return {
    sheetName: `${domain}-sheet`,
    domain,
    confidence: 1.0,
    rationale: 'test',
    disagreed: false,
...
function columnMap(totalCols: number, mappedCols: number): SheetColumnMap {
  return {
...
const LOCK: HeaderLock = {
  sheetName: 'GL Product Framework',
  headerRowIndex: 0,
...
const REVIEW_ITEM: ReviewItem = {
  kind: 'low-confidence-map',
  sheetName: 'GL Product Framework',
  detail: 'coverage.name: confidence 0.6 < threshold',
}

const DISCREPANCY: ValidationDiscrepancy = {
  kind: 'orphan-coverage',
  detail: 'GL.COV.004.009 has no parent GL.COV.004',
}

// ─── Tests ────────────────────────────────────────────────────────────────────

describe('reconcileOutput (stage 6)', () => {
  it('passes all inputs through unchanged', () => {
    ...
    const queue: ReviewItem[] = [REVIEW_ITEM]
    const discrepancies: ValidationDiscrepancy[] = [DISCREPANCY]
  ...
  it('summaryCounts: validatorDiscrepancies count', () => {
    const disc: ValidationDiscrepancy[] = [DISCREPANCY, DISCREPANCY, DISCREPANCY]
  ...
  it('entitiesProduced equals entities.length (consistent with rowsExtracted)', () => {
