...
def ensure_topics():
    try:
        import kafka as kafka_mod  # type: ignore[import-not-found]
        ...
        # Best-effort safeguard against local shadowing; ignore static analysis for __file__.
        mod_path = getattr(kafka_mod, '__file__', '')  # type: ignore[attr-defined]
        if mod_path and 'site-packages' not in mod_path:
            print(f"[topics] ERROR: Imported local shadowing module at {mod_path}. Rename conflicting folder or adjust PYTHONPATH.")
            return 1
    except ImportError:
