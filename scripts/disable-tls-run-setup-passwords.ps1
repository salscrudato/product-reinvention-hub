...
foreach ($line in $lines) {
    ...
    if ($inXpack) {
        # replace enabled true under transport.ssl or http.ssl
        if ($line -match 'transport:\s*$') { $out += $line; continue }
        if ($line -match 'http:\s*$') { $out += $line; continue }
        if ($line -match 'ssl:\s*$') { $out += $line; continue }
        $trim = $line.Trim()
        if ($trim -match '^enabled:\s*true$') { $out += ($line -replace 'true','false') ; continue }
        $out += $line
    } else {
        $out += $line
    }
...
Write-Host "Wrote modified elasticsearch.yml with TLS disabled (backup at $bak)" -ForegroundColor Green

Write-Host "Restarting Elasticsearch to pick up changes" -ForegroundColor Yellow
...
Write-Host "Done. Inspect scripts\es-setup-passwords-output.txt for generated passwords." -ForegroundColor Cyan
