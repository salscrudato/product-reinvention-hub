"""Replay JSONL spool file into Kafka (consolidated)."""
...
def main() -> int:
	p = argparse.ArgumentParser(description="Replay spooled JSONL events into Kafka raw topic")
	p.add_argument("--file", default="event_spool.jsonl", help="Path to spool file (default: event_spool.jsonl)")
	p.add_argument("--topic", default=RAW_TOPIC, help="Kafka topic (default from env or crew-raw-events)")
	p.add_argument("--batch", type=int, default=500, help="Flush every N messages (default 500)")
	p.add_argument("--sleep", type=float, default=0.0, help="Sleep seconds between messages (simulate real-time)")
	args = p.parse_args()
	try:
