"""Generic Kafka JSON producer utility.

Features:
 - Reads JSON objects from a file (JSONL) or STDIN.
 - Can generate dummy events if no input file is provided.
 - Optional key field extraction for partition affinity.
 - Adjustable delay between messages.
 - Graceful degradation if kafka-python is NOT installed.

Environment variables (can be overridden by CLI):
  KAFKA_BOOTSTRAP (default: localhost:9092)
  KAFKA_RAW_TOPIC (default: crew-raw-events)
"""
