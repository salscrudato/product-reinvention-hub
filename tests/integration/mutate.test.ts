...
describe('GTM launch tracker — project + task + seed batch (emulator)', () => {

  it('project create: entity(rev:1) + server-minted PRJ refId + audit + version + searchIndex', async () => {
    await adapter.db.mutate({
      op: 'create', path: 'projects/PJ1', entityType: 'project', actor,
      data: {
        name: 'Personal Auto — National Launch', description: '',
        targetLaunchDate: '2026-12-31', status: 'planning',
        owner: { uid: actor.uid, name: actor.name },
      },
    })

    const p = await get('projects/PJ1')
    expect(p?.rev).toBe(1)
    expect(p?.name).toBe('Personal Auto — National Launch')
    expect(String(p?.refId)).toMatch(/^PRJ\.\d{3}$/)   // minted at the seam (no LOB prefix)

    // project is now an INDEXABLE type → a ⌘K entry lands in the SAME transaction
    const idx = await get('searchIndex/projects_PJ1')
    expect(idx?.type).toBe('project')
    expect(idx?.title).toBe('Personal Auto — National Launch')

    const audits = await list('auditEvents')
    expect(audits.some(a => a.entityPath === 'projects/PJ1' && a.action === 'create')).toBe(true)
    const versions = await list('versions')
    expect(versions.some(v => v.entityPath === 'projects/PJ1')).toBe(true)
  })

  it('mutateBatch seeds many tasks atomically — each with rev:1 + audit + version + searchIndex', async () => {
    const payloads = Array.from({ length: 5 }, (_, i) => ({
      op: 'create' as const, path: `tasks/seed-${i}`, entityType: 'task', productId: 'HO.PROD.001', actor,
      data: { title: `Seeded task ${i}`, column: 'IDEATION', projectId: 'PJ1', origin: 'seeded', dueAt: '2026-12-31' },
    }))
    await adapter.db.mutateBatch(payloads)

    for (let i = 0; i < 5; i++) {
      const t = await get(`tasks/seed-${i}`)
      expect(t?.rev).toBe(1)
      expect(t?.projectId).toBe('PJ1')
      expect(await get(`searchIndex/tasks_seed-${i}`)).toBeTruthy()
    }
    const audits = await list('auditEvents')
    expect(audits.filter(a => String(a.entityPath).startsWith('tasks/seed-') && a.action === 'create')).toHaveLength(5)
    const versions = await list('versions')
    expect(versions.filter(v => String(v.entityPath).startsWith('tasks/seed-'))).toHaveLength(5)
  })

  it('re-seed via mutateBatch clears prior seeded tasks then creates new ones (mixed delete + create)', async () => {
    await adapter.db.mutateBatch([
      { op: 'create', path: 'tasks/old-1', entityType: 'task', actor, data: { title: 'Old', column: 'IDEATION', projectId: 'PJ1', origin: 'seeded' } },
    ])
    expect(await get('tasks/old-1')).toBeTruthy()

    await adapter.db.mutateBatch([
      { op: 'delete', path: 'tasks/old-1', entityType: 'task', actor },
      { op: 'create', path: 'tasks/new-1', entityType: 'task', actor, data: { title: 'New', column: 'BUILD_FILE', projectId: 'PJ1', origin: 'seeded' } },
    ])
    expect(await get('tasks/old-1')).toBeNull()
    expect(await get('searchIndex/tasks_old-1')).toBeNull()   // ⌘K entry removed too
    expect((await get('tasks/new-1'))?.title).toBe('New')

    const audits = await list('auditEvents')
    expect(audits.some(a => a.entityPath === 'tasks/old-1' && a.action === 'delete')).toBe(true)
  })

  it('completing a task is an audited, rev-guarded update (done + completedAt)', async () => {
    await adapter.db.mutate({ op: 'create', path: 'tasks/tc1', entityType: 'task', actor, data: { title: 'Finish', column: 'IDEATION', projectId: 'PJ1', origin: 'seeded', done: false, completedAt: null } })
    await adapter.db.mutate({ op: 'update', path: 'tasks/tc1', entityType: 'task', actor, expectedRev: 1, data: { done: true, completedAt: '2026-07-08T00:00:00.000Z' } })

    const t = await get('tasks/tc1')
    expect(t?.done).toBe(true)
    expect(t?.rev).toBe(2)
    const audits = await list('auditEvents')
    expect(audits.filter(a => a.entityPath === 'tasks/tc1')).toHaveLength(2)   // create + update
  })
})
