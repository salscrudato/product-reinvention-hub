...
|---|---|---|---|
| **import-hardening (IH)** | `server/lib/import-brain/**` (stages 1-7 + constants + prompts), `shared/src/insurance/lobRegistry.ts` (`refIdSegmentKind`), `shared/src/import/brain-server-entry.ts`, `server/lib/import-brain-shared.cjs`, `samples/hardening/**`, `scripts/{hardening-manifest,import-enumerate}.mjs`, `tests/import-brain/hardening-p0-*.test.ts`, `docs/import-hardening/**` | ✅ **IH1 done** — baseline + triage + P0 silent-loss fixes. F01–F20 verified at `2b1f893` (verdicts + file:line in [docs/import-hardening/BASELINE.md](docs/import-hardening/BASELINE.md); machine-readable priority-scored [ledger.json](docs/import-hardening/ledger.json)). **Fixed + fixture-locked (each red at 2b1f893):** F01 multi-refId overwrite (occurrence keys, prompt no-split, single post-consensus expansion w/ preserved evidence), F02/F03 kind-disagreement → ladder + judge sees all candidates, F04 header re-score on the authoritative grid (preamble fixture), F05 rowKind via shared registry parser, F06 conservation (extra products/programs → BLOCKING unresolved, proposed=accepted+unresolved asserted), F07/F08 deterministic citation resolver + severity policy (BLOCKING prevents auto-accept; LLM validator = semantics-only WARN; fixed latent citation-row drift via absolute gridRows), F16 parseWithRetry+sanitizeEntities on every stage parse (telemetry + one targeted retry, never silent nulls). **Corpus:** samples/hardening (29 workbooks/218 sheets manifest ground truth; CORE Rule References 2280×29 exceeds embed cap), offline enumeration 24/24 complete 0 unhandled ([RESULTS/corpus-baseline.json](docs/import-hardening/RESULTS/corpus-baseline.json) — NOT gated yet, IH2 gates it). **Open queue for IH2 (priority order):** PCM-A attachment fold (terms never assembled → UI dashes, cost L), PCM-B history read path (db.list can't return version docs), F10 stop_reason/truncation detection, F09 embed-cap continuation, F17 remap graph rewrite, F18 filing PH hardcoding, F20 eval linkage metrics + fabrication penalty + non-circular goldens, F12/F13/F14/F11/F15. F19/PCM-C disproven (batch DOES write version zero — do not re-fix). ⚠️ IH2: first live eval run confirms stage-4 prompt-contract parity (no-split change); expect F1 ≥ prior since expansion moved, not removed. | `ecec10f` (run 2473 ✅) |
