...
| **public-surfaces (F1·A)** | `app/src/components/icons/**`, `app/src/components/ui/icons.tsx` (shim), `app/src/lib/pricing.ts`(+test), `app/src/routes/{Pricing,Landing,SharedView}.tsx`, `app/src/App.tsx`, `app/src/index.css` (additive only), `server/lib/share.js` (+1 mount line in `server/server.js`). See [docs/audit/EXECUTION-A.md](docs/audit/EXECUTION-A.md). | 🔄 In progress — icon registry + pricing + landing nav (wave 1), then share seam (wave 2). Egg + RISK-013 found already-shipped → verify only. Please don't edit these app files until done. | — |
