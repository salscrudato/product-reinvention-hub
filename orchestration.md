...
| **cx-wave (R0)** | Customer-experience wave across 7 surfaces: `app/src/components/ai/StreamRenderer.tsx` (upgrade in place), `app/src/components/ai/WaveformLoader.tsx` (new), `app/src/components/shell/{Sidebar,Topbar}.tsx` + search overlay, Home right-pane collapse + AI task summary (new `server/lib/ai/` handler on MID_REASONER), `app/src/routes/{Products,Explorer-equivalent,Tasks,News}.tsx`, tasks/gtm seed review (editable SLA + seed-button root cause), News 5-day real backfill + images (`server/lib/ai/refresh-news.js`, `server/lib/storage.js`), Claims renderer structure, `app/src/import/UnifiedImportModal.tsx` + `AgentVisualizer.tsx` (enhance in place, no forks) + write-phase batching within the mutate envelope. ≤2 new semantic tokens in `index.css`. See [docs/audit/EXECUTION-R0.md](docs/audit/EXECUTION-R0.md). | 🔨 **In progress** — building in local wave commits; **PUSH HELD by user directive** (build → wait for explicit go to push/deploy/live-verify). Live steps deferred: 5-day news backfill run, isolated-tenant workbook+filing import walk, write-phase before/after timing on dev. | local (unpushed) |
