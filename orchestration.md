...
| **public-surfaces (F1·A)** | `app/src/components/icons/**`, `app/src/components/ui/icons.tsx` (shim), `app/src/lib/pricing.ts`(+test), `app/src/routes/Pricing.tsx`, `app/src/App.tsx` (added `/pricing` route line only), `app/src/index.css` (additive only). Wave 2: `server/lib/share.js` (+1 mount line in `server/server.js`) + `app/src/routes/SharedView.tsx`. See [docs/audit/EXECUTION-A.md](docs/audit/EXECUTION-A.md). | 🔄 Wave 1 gated + pushing: icon registry promoted to `components/icons/` (+7 F3 glyphs + `IconShare`); `ui/icons.tsx` now a re-export shim; new public `/pricing` page (4 commercial layers + hand-rolled-SVG ROI calc) + `lib/pricing.ts` (ILLUSTRATIVE) + ROI test. Egg + RISK-013 found **already shipped** → verified only, not rebuilt. ⚠️ **Landing.tsx is being rewritten by another lane — Lane A did NOT touch it.** **ASK to the landing lane:** add `<Link to="/pricing">Pricing</Link>` to the header nav + footer (the route is live; I stayed off your file to avoid clobbering your rewrite). | — |
