...
// ─── ONE-SHOT CORPUS SEED (gated — remove after seeding) ────────────────────
// Enabled only when SEED_CORPUS_ENABLED=1 is set in App Service config.
// The endpoint runs on the App Service (inside the Cosmos firewall) so it can
// write seed data regardless of the caller's network location.
// Remove this block + unset the env var once all tenant corpora are populated.
if (process.env.SEED_CORPUS_ENABLED === '1') {
  ...
  console.log('[prodhub-host] /api/admin/seed-corpus mounted (SEED_CORPUS_ENABLED=1)')
}
