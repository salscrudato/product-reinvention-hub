...
// platform-shared.cjs is generated by `pnpm build:platform` (esbuild bundle of
// shared/src/platform/server-entry.ts) and COMMITTED. deriveDraftIdentity /
// deriveDraftReadiness are pure read-time projections (identity from persisted
// lineage + lob; readiness verdict from the persisted import summary) attached to
// product rows — never persisted themselves.
const { deriveDraftIdentity, deriveDraftReadiness } = require('./platform-shared.cjs')
...
// isProductPath / identity enrichment: product rows carry server-derived, read-only
// `identity` (displayName "[artifactBase] - [LOB] - [Mon DD]", sourceFileName,
// importedAt, contentHash) and `readiness` (citations/blockers/validation/promotable
// verdict from the persisted import summary) projections computed at read time —
// legacy drafts need no migration and NOTHING here writes. Both are derived last so
// a (hypothetical) stored field of the same name can never spoof the projection.
const isProductDoc = (path) => { const s = segs(path); return s.length === 2 && s[0] === 'products' }
const withIdentity = (row, data) => ({ ...row, identity: deriveDraftIdentity(data), readiness: deriveDraftReadiness(data) })
...
router.get('/get', requireCapability('product:read'), requireTenant, async (req, res) => {
  ...
  const row = ent ? { id, ...ent.data } : null
  res.json({ data: row && isProductDoc(ent.path) ? withIdentity(row, ent.data) : row })
})
...
router.post('/list', requireCapability('product:read'), requireTenant, async (req, res) => {
  ...
  // Product rows carry the derived read-only identity projection (see /get above).
  const isProducts = String(path || '') === 'products'
  const toRow = (r) => {
    const row = { id: segs(r.path).at(-1), ...r.data }
    return isProducts ? withIdentity(row, r.data) : row
  }
  if (paged) {
  ...
  res.json({ data: resources.map(toRow) })
})

// ─── Draft dedup lookup (task 2 — read-only) ─────────────────────────────────
// GET /drafts/dedup?contentHash=… — tenant-scoped: which existing drafts came from
// an artifact with this content hash? Returns { matches: [{ id, displayName,
// importedAt, status }] } — [] when none. The import-apply path stamps
// data.contentHash at draft creation (P3, app/src/lib/import — sha256 of the raw
// uploaded bytes, `sha256:<hex>`; multi-file batches hash the sorted per-file
// digests); drafts persisted before that wave have no hash and never match.
const CONTENT_HASH_RE = /^[A-Za-z0-9:_-]{8,128}$/
router.get('/drafts/dedup', requireCapability('product:read'), requireTenant, async (req, res) => {
  const contentHash = String(req.query.contentHash || '')
  if (!CONTENT_HASH_RE.test(contentHash)) return res.status(400).json({ error: 'invalid_content_hash' })
  const tid = resolveTenantForPrincipal(req.user)
  try {
    const sql = "SELECT TOP 20 c.path, c.data FROM c WHERE c.kind='entity' AND c.coll='products' AND c.tenantId=@tid AND c.data.contentHash=@hash"
    const { resources } = await storeFor(tid).items.query(
      { query: sql, parameters: [{ name: '@tid', value: tid }, { name: '@hash', value: contentHash }] },
      { maxItemCount: 20 },
    ).fetchAll()
    const matches = resources
      .map((r) => {
        const idn = deriveDraftIdentity(r.data)
        const status = r.data?.lifecycle ?? r.data?.lifecycleState ?? null
        return { id: segs(r.path).at(-1), displayName: idn.displayName, importedAt: idn.importedAt, status: typeof status === 'string' ? status : null }
      })
      .sort((a, b) => String(b.importedAt ?? '').localeCompare(String(a.importedAt ?? '')) || a.id.localeCompare(b.id))
    res.json({ matches })
  } catch {
    res.status(503).json({ error: 'dedup_unavailable' })
  }
})

// ─── Draft promote (P3) — the armed gate from Drafts to the portfolio ────────
// POST /drafts/:id/promote { expectedRev? } — server-verdict promotion. Derives
// readiness from the PERSISTED import summary and refuses with
// 409 { error:'not_promotable', blockers } (verbatim reasons) when evidence blocks;
// otherwise flips lifecycle → LAUNCHED through the same atomic envelope as every
// write (audit-chained, versioned, source '/api/db/drafts/promote'). Enforcement
// itself lives in envelope()'s NOT_PROMOTABLE guard — a direct /mutate flip hits the
// identical verdict — this route is the contract that REPORTS the blockers cleanly.
router.post('/drafts/:id/promote', requireCapability('product:write'), requireTenant, async (req, res) => {
  const tid = resolveTenantForPrincipal(req.user)
  const actor = { uid: req.user.uid, name: req.user.name }
  const path = `products/${req.params.id}`
  const expectedRev = typeof req.body?.expectedRev === 'number' ? req.body.expectedRev : undefined
  try {
    const current = await readEntity(tid, path)
    if (!current) return res.status(404).json({ error: 'not_found' })
    if (expectedRev !== undefined && (current.rev ?? 0) !== expectedRev) return res.status(409).json({ error: 'stale_rev', currentRev: current.rev ?? 0 })
    const lifecycle = current.data?.lifecycle ?? current.data?.lifecycleState ?? null
    if (lifecycle === 'LAUNCHED') return res.status(409).json({ error: 'already_launched' })
    const readiness = deriveDraftReadiness(current.data)
    if (!readiness.promotable) return res.status(409).json({ error: 'not_promotable', blockers: readiness.blockers })
    const payload = { op: 'update', path, entityType: 'product', data: { lifecycle: 'LAUNCHED', status: 'ACTIVE', reviewStatus: 'APPROVED' }, expectedRev }
    const { rev } = await commitEnvelope(tid, () => envelope(tid, payload, actor, '/api/db/drafts/promote')())
    ...
    log.error('data', 'promote_failed', { tenantId: tid, path, detail: String(e.message || e).slice(0, 200) })
    res.status(500).json({ error: 'promote_failed', detail: String(e.message || e) })
  }
})
...
function envelope(tid, payload, actor, source) {
  ...
  return async () => {
    ...
    // Readiness is WRITE-ONCE (P3 hostile-review fix): the import-apply create stamps
    // it; after that no write may alter it, so the promote verdict cannot be laundered
    // away by re-stamping a clean summary in one or two writes. An identical re-send
    // (full-doc editors, /vote's read-modify-write) passes; a restore is a rewind of
    // an already-audited state and op:'restore' replaces the doc verbatim, so it is
    // not an authoring path for a new summary.
    if (entityType === 'product' && op === 'update' && current && 'readiness' in data
      && JSON.stringify(data.readiness ?? null) !== JSON.stringify(current.data?.readiness ?? null)) {
      const e = new Error('readiness_immutable'); e.code = 'READINESS_IMMUTABLE'; throw e
    }
    // Promote gate (P3): flipping a product to LAUNCHED is enforced HERE, at the single
    // write choke point — /mutate, /mutateBatch, /restore and /drafts/:id/promote all
    // pass through. The verdict derives from the PERSISTED readiness summary AND the
    // effective post-write state (so a fresh create-as-LAUNCHED carrying a blocked
    // summary is refused too). Legacy drafts with no summary are not blocked
    // (flag-not-invent — absence of evidence never fabricates a blocker).
    if (entityType === 'product' && op !== 'delete' && data.lifecycle === 'LAUNCHED') {
      const prior = current?.data?.lifecycle ?? current?.data?.lifecycleState ?? null
      if (prior !== 'LAUNCHED') {
        const readiness = deriveDraftReadiness(current?.data)
        const effectiveData = op === 'update' && current?.data ? { ...current.data, ...data } : data
        const effective = deriveDraftReadiness(effectiveData)
        const verdict = !readiness.promotable ? readiness : !effective.promotable ? effective : null
        if (verdict) { const e = new Error('not_promotable'); e.code = 'NOT_PROMOTABLE'; e.blockers = verdict.blockers; throw e }
      }
    }
...
router.post('/restore', requireCapability('product:write'), requireTenant, async (req, res) => {
  ...
  try {
    ...
    // The promote gate fires on restores too (a rewind forward to a LAUNCHED state is
    // still a promotion of a currently-blocked draft) — surface the structured verdict,
