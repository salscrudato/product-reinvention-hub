...
// shared/src/platform/server-entry.ts) and COMMITTED. deriveDraftIdentity is the
// pure read-time projection (displayName / sourceFileName / importedAt / contentHash
// from the persisted lineage + lob) attached to product rows — never persisted.
const { deriveDraftIdentity } = require('./platform-shared.cjs')
...
// isProductPath / identity enrichment: product rows carry a server-derived, read-only
// `identity` projection (displayName "[artifactBase] - [LOB] - [Mon DD]", sourceFileName,
// importedAt, contentHash) computed at read time from the persisted lineage — legacy
// drafts need no migration and NOTHING here writes. `identity` is derived last so a
// (hypothetical) stored field of the same name can never spoof the projection.
const isProductDoc = (path) => { const s = segs(path); return s.length === 2 && s[0] === 'products' }
const withIdentity = (row, data) => ({ ...row, identity: deriveDraftIdentity(data) })
...
// importedAt, status }] } — [] when none. NOTE (recorded storage delta): no write
// path stamps data.contentHash today, so this returns [] until the import-apply
// path persists it; the endpoint + seam ship now so that write-side wave has a
