// personalHome.ts — Personal Home seed constants (ISO-style HO-3 Special Form).
// All refIds use the PH.* prefix. The rating algorithm is identical to the legacy
// HO-3 seed — the $1,528 canary is preserved exactly. This replaces ho3.ts.
import type {
...
export const PH_FOOTPRINT_STATES = PH_LOB.footprintStates

const SEC_I  = PH_LOB.sections[0]!.shortName   // 'Section I'
const SEC_II = PH_LOB.sections[1]!.shortName   // 'Section II'

const COASTAL_CONSTRAINT_NOTE =
  `Coastal states only (${PH_LOB.peril.eligibleStates.join(' ')}); dollar amount must be ≥ all-peril deductible`
const COASTAL_RULE_OUTCOME =
  `Coastal states only (${PH_LOB.peril.eligibleStates.join(' ')}); dollar amount ≥ all-peril deductible`
...
const FOOTPRINT_SCOPE = { allStates: false, states: [...PH_FOOTPRINT_STATES] }
const COASTAL_SCOPE   = { allStates: false, states: [...PH_LOB.peril.eligibleStates] }

// ─── Product ─────────────────────────────────────────────────────────────────

export const PH_PRODUCT: Omit<Product, 'createdAt' | 'updatedAt'> & {
...
export const PH_LD_TABLES: Record<string, LDTable> = {
  'PH.LD.001': {
...
export const PH_RT_TABLES: Record<string, RTTable> = {
  'PH.RT.001': {
...
export function makePHRtGetter(tables: Record<string, RTTable>): RtGetter {
  return (tableRef: string, q: Record<string, unknown>): number => {
    ...
    // Grid-managed tables resolve through the generic N-D lookup; it returns null for
    // every legacy/seeded table (no `dimensions`), so the bespoke lookups below —
    // and the $1,528 canary — are untouched.
...
export const PH_RATING_PROGRAM: Omit<RatingProgram, 'createdAt' | 'updatedAt'> & {
  ...
  steps: [
    ...
    { id: 's8a', order: 9,  label: 'Replacement Cost endorsement factor',     op: 'MUL',       source: { type: 'CONST', value: 1.10 },                                      condition: 'rcElected', roundTo: 2 },
...
export const PH_COVERAGES: CoverageSeed[] = [
  {
  ...
  {
    refId: 'PH.COV.004', name: 'Coverage D — Loss of Use',
  ...
  {
    refId: 'PH.COV.005', name: 'Coverage E — Personal Liability',
    parentId: null, order: 5, requirement: 'MANDATORY',
    claimsBasis: 'Occurrence', premiumGenerating: true, source: 'BUREAU',
    formNumbers: ['HO 00 03'],
    terms: [{ id: 'cov-e-limit', kind: 'LIMIT', label: 'Coverage E Limit', basis: 'per occurrence', ldTableRef: 'PH.LD.001', default: 300000, unit: 'dollars' }],
    ...FOOTPRINT_SCOPE, ...gov(),
  },
  {
    refId: 'PH.COV.006', name: 'Coverage F — Medical Payments',
    parentId: null, order: 6, requirement: 'MANDATORY',
    claimsBasis: 'Occurrence', premiumGenerating: true, source: 'BUREAU',
    formNumbers: ['HO 00 03'],
    terms: [{ id: 'cov-f-limit', kind: 'LIMIT', label: 'Coverage F Limit', basis: 'per person per occurrence', ldTableRef: 'PH.LD.002', default: 1000, unit: 'dollars' }],
    ...FOOTPRINT_SCOPE, ...gov(),
  },
  {
    refId: 'PH.COV.001.001', name: 'Water Back-Up & Sump Overflow',
    parentId: 'PH.COV.001', order: 1, requirement: 'OPTIONAL',
    claimsBasis: 'Occurrence', premiumGenerating: true, source: 'BUREAU',
    formNumbers: ['HO 04 95'],
    terms: [{ id: 'water-backup-limit', kind: 'LIMIT', label: 'Water Back-Up Limit', basis: 'per occurrence', ldTableRef: 'PH.LD.006', default: 5000, unit: 'dollars' }],
    ...FOOTPRINT_SCOPE, ...gov(),
  },
  {
    refId: 'PH.COV.002.001', name: 'Other Structures — Increased Limits',
    parentId: 'PH.COV.002', order: 1, requirement: 'OPTIONAL',
  ...
  {
    refId: 'PH.COV.003.001', name: 'Personal Property Replacement Cost',
    parentId: 'PH.COV.003', order: 1, requirement: 'OPTIONAL',
    claimsBasis: 'Occurrence', premiumGenerating: true, source: 'BUREAU',
  ...
  {
    refId: 'PH.COV.003.002', name: 'Scheduled Personal Property',
    parentId: 'PH.COV.003', order: 2, requirement: 'OPTIONAL',
    claimsBasis: 'Occurrence', premiumGenerating: true, source: 'BUREAU',
...
export const PH_FORMS: FormSeed[] = [
  {
...
export const PH_RULES: RuleSeed[] = [
  { refId: 'PH.RU.001', category: 'PRODUCT', subCategory: 'Eligibility',
  ...
  { refId: 'PH.RU.002', category: 'PRODUCT', subCategory: 'Coverage Limits',
    condition: 'Coverage B default limit',
    outcome: 'Default = 10% of Coverage A; increase only via HO 04 48',
    ldTableRef: undefined, coverageRefIds: ['PH.COV.002'], formNumbers: ['HO 04 48'],
    ...FOOTPRINT_SCOPE, ...gov() },
  { refId: 'PH.RU.003', category: 'PRODUCT', subCategory: 'Coverage Limits',
    condition: 'Coverage C percentage of A',
    outcome: 'Options per PH.LD.005; default 50% of A',
    ldTableRef: 'PH.LD.005', coverageRefIds: ['PH.COV.003'], formNumbers: [],
    ...FOOTPRINT_SCOPE, ...gov() },
  { refId: 'PH.RU.004', category: 'PRODUCT', subCategory: 'Coverage Limits',
    condition: 'Coverage D limit',
    outcome: '30% of Coverage A (calculated)',
    coverageRefIds: ['PH.COV.004'], formNumbers: [], ...FOOTPRINT_SCOPE, ...gov() },
  { refId: 'PH.RU.005', category: 'PRODUCT', subCategory: 'Coverage Limits',
    condition: 'Coverage E limit options',
    outcome: 'Options per PH.LD.001; default $300,000',
    ldTableRef: 'PH.LD.001', coverageRefIds: ['PH.COV.005'], formNumbers: [],
    ...FOOTPRINT_SCOPE, ...gov() },
  { refId: 'PH.RU.006', category: 'PRODUCT', subCategory: 'Coverage Constraints',
    condition: 'Coverage F $5,000 limit selected',
    outcome: 'Requires Coverage E ≥ $300,000',
    ldTableRef: 'PH.LD.002', coverageRefIds: ['PH.COV.005','PH.COV.006'], formNumbers: [],
    ...FOOTPRINT_SCOPE, ...gov() },
  { refId: 'PH.RU.007', category: 'RATING', subCategory: 'Deductibles',
    condition: 'All-peril deductible selection',
    outcome: 'Options per PH.LD.003; default $1,000',
    ldTableRef: 'PH.LD.003', coverageRefIds: [], formNumbers: [],
    ...FOOTPRINT_SCOPE, ...gov() },
  { refId: 'PH.RU.008', category: 'RATING', subCategory: 'Deductibles',
    condition: 'Wind/Hail percentage deductible elected',
    outcome: COASTAL_RULE_OUTCOME,
    ldTableRef: 'PH.LD.004', coverageRefIds: [], formNumbers: ['HO 03 12'],
    ...COASTAL_SCOPE, ...gov() },
  { refId: 'PH.RU.009', category: 'RATING', subCategory: 'Premium Floor',
    condition: 'Calculated premium',
    outcome: 'Minimum policy premium $500 (PH.RAT.1 step 11)',
    coverageRefIds: [], formNumbers: [], ...FOOTPRINT_SCOPE, ...gov() },
  { refId: 'PH.RU.010', category: 'PRODUCT', subCategory: 'Eligibility',
...
export const PH_FORM_RULES: FormRuleSeed[] = [
...
export const PH_DICTIONARY: DictSeed[] = [
  { refId: 'PH.DEF.001', name: 'Named Insured', type: 'TEXT',
  ...
  { refId: 'PH.DEF.002', name: 'Property Address', type: 'TEXT',
  ...
  { refId: 'PH.DEF.003', name: 'Coverage A Amount', type: 'CURRENCY',
  ...
  { refId: 'PH.DEF.004', name: 'All-Peril Deductible', type: 'CURRENCY',
  ...
  { refId: 'PH.DEF.005', name: 'Protection Class', type: 'LIST',
  ...
  { refId: 'PH.DEF.006', name: 'Construction Type', type: 'LIST',
  ...
  { refId: 'PH.DEF.007', name: 'Territory Code', type: 'LIST',
  ...
  { refId: 'PH.DEF.008', name: 'Appraised Value', type: 'CURRENCY',
  ...
  { refId: 'PH.DEF.009', name: 'Device Type', type: 'LIST',
  ...
  { refId: 'PH.DEF.010', name: 'Effective Date', type: 'DATE',
...
// ─── Default task templates (SLA set) ─────────────────────────────────────────

// A curated ~18-activity subset of the P&C "Product Manufacture" value taxonomy
// (reference_tasks / Process Value Explorer): the most common Level-4 activities per
// phase, mapped onto the 4 lifecycle columns and grouped by their Level-2/3 category.
// Every new product offers these for selection; the PM deselects what doesn't apply.
...
export const PH_DEFAULT_TASK_TEMPLATES = DEFAULT_TASK_TEMPLATES

// ─── Sample users ─────────────────────────────────────────────────────────────

export const PH_SEED_USERS: Array<Omit<User, 'createdAt'> & { createdAt: null; password: string }> = [
  {
  ...
  {
    email: 'peter@productreinvention.app', name: 'Peter',
    role: 'ADMIN', active: true, mustChangePassword: false,
    password: 'jones', createdAt: null,
  },
]

// ─── Sample feedback ──────────────────────────────────────────────────────────

export const PH_SAMPLE_FEEDBACK: Array<Omit<Feedback, 'createdAt' | 'updatedAt'> & { createdAt: null; updatedAt: null }> = [
  {
...
// 700 → ×1.05 = 735 → ×1.30 = 955.50 → 956 → ×1.00 = 956 → ×1.06 = 1013.36
// → +24 = 1037.36 → +6 = 1043.36 → ×1.10×1.00 = 1147.70 → ×1.10 = 1262.47
// → +75 = 1337.47 → +190.50 SPP = 1527.97 → MAX(·,500) round 0 = 1528

export const PH_WORKED_EXAMPLE: RatingInputs = {
  territory:           'T002',
  pc:                  5,
  construction:        'M',
  covA:                400000,
